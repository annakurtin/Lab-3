...
boxplot(deer_w2~usedFactor, data=bow_wolf, main = "Bow Valley:Deer", xlab="usedFactor", ylab="deer")
boxplot(deer_w2~usedFactor, data=redv_wolf, main = "Red Deer:Deer", xlab="usedFactor", ylab="deer")
...
Could be confounded or colinear variables. 
...
par(mfrow = c(2,3))
boxplot(deer_w2~usedFactor, data=bvkde, main = "Deer", xlab="usedFactor", ylab="deer",col="light blue")
boxplot(elk_w2~usedFactor, data=bvkde, main = "Elk", ylab="elk_w2", xlab="usedFactor",col="light blue")
boxplot(moose_w2~usedFactor, data=bvkde, main = "Moose", ylab="moose_w2", xlab="usedFactor",col="light blue")
boxplot(goat_w2~usedFactor, data=bvkde, main = "Goat", ylab="goat_w2", xlab="usedFactor",col="light blue")
boxplot(sheep_w2~usedFactor, data=bvkde, main = "Sheep", ylab="sheep_w2", xlab="usedFactor",col="light blue")
```

Now lets do for Elevation and Distance from Human Access2
```{r}
par(mfrow = c(1,3))
boxplot(Elevation2~usedFactor, data=bvkde, main = "Elevation", xlab="usedFactor", ylab="deer",col="light blue")
boxplot(DistFromHumanAccess2~usedFactor, data=bvkde, main = "Dist. Human Access", ylab="elk_w2", xlab="usedFactor",col="light blue")
boxplot(DistFromHighHumanAccess2~usedFactor, data=bvkde, main = "Dist. High Human Access", ylab="elk_w2", xlab="usedFactor",col="light blue")
```
...
Looks to be far away from high human activity and have less of an effect of elevation. 
...
# we see that there are differences in the availability, we also see that there is a difference between the distance from the availability mean and the used mean between the packs 
# we want selection which is a function of what you want and what is available to you
...
##"pretty ugly graph to put it all together"
...
# Why tibble???????????????????????????
## convert it to a tibble bc tidyverse needs it
bow_wolf <- wolfkde2 %>% filter(pack == "Bow Valley")
red_wolf <- wolfkde2 %>% filter(pack == "Red Deer")
...
rbinom(100, 1, 0.5)
# use rbinom to simulate random data 
...
sum(surv1)
#.02% chance of all grandkids boys 
# how many potential rows of 6 are there in 100 trials? 100-6

# Goal of statistics is to estimate the population probability based on the given data (an observed "trial") - use confidence intervals, etc. 
```
...
plogis is the logistic link function (B0 + B1x)
```{r}
plogis(0)
...
*Proof from notes: the logit link function coerces the output of p to be between o and 1

```{r}
e <- 2.71

# Large B1: output of P should be coerced to be large
e^100/(1+e^100)
## output is 1
...
e^.0000001/(1+e^.0000001)
## output is .5?

e^.5/(1+e^.5)
## ouptput is .62?

## how exactly is this constraining the output to between zero and 1?
log(2/1)
...
# simulate each number to create data
## take linear expression: 0+0.07*x
## for each increase in habitat quality, the probability of presence increases by .07
...
# change plogis intercept to -1, the baseline probability of observing a 1 decreases 

# if you change the slope value to .21, the relationship becomes a lot tighter
```
...
# intercept is baseline probability which is .50 ish, which is correct
# x in our simulation was .07, but the x in this linear model is .012, means that it is fitting the results improperly
```
...
summary(res)
yLogit=predict(res)
# hard to interpret the logic link function bc the resultant value is log odds 

plot( yLogit ~ x )
# plots linear predictor on y axis (B0+B1x)
yhat=predict(res, type="response")
# transform it through logit link to get response
plot( y ~ x)
lines(yhat~x)
# This is the true logistic regression 
# slope of derivative at infleciton point is the linear model fit, but in this case it changes as it gets to the tails 
```
...
summary(elev)
#9.348 intercept: gives you likelihood/prob of use at 0 elev
# elevation slope coeff tells you the effect in the term of log odds
...
summary(distHHuman)
# can only compare outputs fro m regression if they are in the same units (dist from humans and elev)
...
# do univariate analyses with box plots to make sure you're interpreting your outputs correctly 
```
...
elev <- glm(used ~ Elevation2, family=binomial(logit), data=wolfkde)
summary(elev)
wolfkde$fitted.Elev <- fitted(elev) # fitted same as predict but you don't have to specify type = response
# Gives you predicted probabilities of wolf use as a function of the change in one value 
# R^2 is strength of correlation b/w prediction and response 
head(wolfkde)
...
After looking at univariate variables, plot 
...
## Plotting Logistic Regresson with ggplot2 - How Mark analyzes things 
...
# stat_smooth fits a univeriate logistic regression with family binomial 
```
...
```
This is the first analysis that shows us the two wolf packs respond very differently to elevation.  (Bow valley pack is avoiding higher elevations more so thatn the red deer pack. I wonder if they respond differently to Moose, or Sheep, say?
...
# flaw with this: these are all univariate models/predictions evaluated in independence of each other 
```
...
figPrey
dev.off()
# give syou univariate predictions form each prey species from an averaged wolf model 
```
